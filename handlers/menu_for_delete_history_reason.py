...
async def delete_history_reason(message: Message, state: FSMContext) -> None:
    ...
    await state.update_data(reasons=reasons,
                            page=1)
    await message.answer(get_text('menu_delete_history_reason', throw_data={'cpage':1, 'allpage': q_page}),
...
async def process_turn_right(query: CallbackQuery, state: FSMContext) -> None:
    ...
    await bot.edit_message_text(
        text=get_text('menu_delete_history_reason', throw_data={'cpage':page, 'allpage': q_page}),
...
@main_router.callback_query(
    Admin.main, F.data.split(":")[0] == "action", F.data.split(":")[1] == "select_reason"
)
async def process_select_reason(query: CallbackQuery, state: FSMContext) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=query.from_user.id,
        message_id=query.message.message_id,
        reply_markup=keyboard_inline.menu_reason(page_num=data['page'], reasons=reasons),
    )


@main_router.callback_query(
    Admin.main, F.data.split(":")[0] == "action", F.data.split(":")[1] == "delete_reason"
)
